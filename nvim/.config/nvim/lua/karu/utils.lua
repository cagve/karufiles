...
py3 << EOF
import os
os.system("echo ']]..yank_text..[['| pandoc -t plain | xclip -selection clipboard")
EOF
endfunction
call s:Test()
    ]]
...
M.extract_section = function()
    ...
    vim.api.nvim_buf_set_keymap(buf, 'n', '<C-c>',
        ':lua require("karu.utils").quit_saving(' .. buf .. ',' .. start_row .. ',' .. end_row .. ')<CR>', {})
end
